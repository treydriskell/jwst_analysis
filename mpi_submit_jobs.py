...
def create_xml(astro_params, outdir, values, param_paths, z):
    # template_xml = '/home/gdriskell/jwst_analysis/xmls/timescale_template.xml' #astro_params['xml_template']
...
def create_jobs_from_list(astro_params, xml_fns, initial,final):
    template_job = '/home/gdriskell/jwst_analysis/jwst_template.job' #astro_params['job_template']
    fn_base = astro_params['param_dir']+'_pi{}_pf{}'.format(initial,final)
    with open(template_job, 'r') as f:
        lines = f.readlines()
    lines[6] = '#SBATCH --job-name=' + fn_base + '\n'
    lines[7] = '#SBATCH --output=/carnegie/nobackup/users/gdriskell/jwst_data/jobs/' + fn_base + '.out\n'
    lines[8] = '#SBATCH --error=/carnegie/nobackup/users/gdriskell/jwst_data/jobs/' + fn_base + '.err\n'
    ...
    job_fn = path.join('/carnegie/nobackup/users/gdriskell/jwst_data/jobs/', fn_base + '.job')
    # print(job_fn)
...
# yaml_fns = ['yamls/paper_params.yaml']
# yaml_fns = ['yamls/test_lower_tau0.yaml']
yaml_fns = ['yamls/smf_params.yaml']
...
for yaml_fn in yaml_fns:#'yamls/nr12_params.yaml' # 'yamls/maxlike_corners.yaml' #'simplified.yaml', 'updated_timescale_params.yaml' #'z0_params.yaml' #'baugh2005_timescale_params2.yaml'#'timescale_sfr_params.yaml' # 'vary_astro_params.yaml'
    ...
    for i, values in enumerate(itertools.product(*pvalues)):
        ...
        outdir = path.join('/carnegie/nobackup/users/gdriskell/jwst_data/', fn_base)
    ...
    # if i in rerun_indices:
        ...
        if (j%params_per_job) == (params_per_job-1):
            ...
            os.chdir('/carnegie/nobackup/users/gdriskell/jwst_data/jobs/')
