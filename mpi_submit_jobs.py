...
def create_job(astro_params, fn_base, xml_fn, z):
    template_job = astro_params['job_template']
    with open(template_job, 'rb') as f:
        lines = f.readlines()
    lines[4] = '#PBS -N ' + fn_base + '\n'
    lines[6] = '#PBS -o logs/' + fn_base + '.out\n'
    lines[15] = '/home/gdriskell/galacticus/Galacticus.exe ' + xml_fn + '\n'
...
def create_jobs_from_list(astro_params, xml_fns, initial,final):
    ...
    for xml_fn in xml_fns:
        ...
        lines.append('/home/gdriskell/galacticus/Galacticus.exe '+ xml_fn + ' &> ' + out_fn  +'\n')
...
yaml_fn = 'yamls/smf_params.yaml' #'yamls/nr12_params.yaml' # 'yamls/maxlike_corners.yaml' #'simplified.yaml', 'updated_timescale_params.yaml' #'z0_params.yaml' #'baugh2005_timescale_params2.yaml'#'timescale_sfr_params.yaml' # 'vary_astro_params.yaml'
...
# if i in rerun_indices:
    ...
    if (j%params_per_job) == (params_per_job-1):
        ...
        output = subprocess.check_output(['qsub', job_fn], text=True)
